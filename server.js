...
app.post('/upload', upload.single('file'), (req, res) => {
    ...
    res.json({
        ...
        filename: filename,
        rawUrl: `${protocol}://${host}/raw/${filename}`
    });
});
...
app.get('/raw/:filename', (req, res) => {
    ...
    const isRoblox = userAgent.includes('Roblox') || userAgent.includes('HttpClient') || userAgent.includes('game:HttpGet') || userAgent.includes('Lua') || userAgent.includes('Synapse') || userAgent.includes('Krnl');
...
app.get('/files', (req, res) => {
    const files = Object.keys(filesDB).map(key => ({
        ...
        rawUrl: `${req.protocol}://${req.get('host')}/raw/${key}`
    }));
    res.json(files);
});
...
app.post('/edit/:filename', express.text(), (req, res) => {
    const filename = req.params.filename;
    const filePath = path.join(uploadDir, filename);
    ...
    res.json({ success: true, message: '✏️ تم تعديل الملف' });
});
